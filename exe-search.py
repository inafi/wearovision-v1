...
cpath=os.path.abspath(os.path.dirname(__file__) + "/gsf") # +/gsf
os.chdir(cpath)
...
def get_dist(ob, freedom):
    ...
    for i in ast.literal_eval(inputs[2]):
        labs.append(i)
    boxes = ast.literal_eval(inputs[1])
    for i in ast.literal_eval(inputs[3]):
        boxes.append(i)
    

    print(ast.literal_eval(inputs[2]))
    ...
    imgl = cpath + "gsf/predictions.jpg"
    pic = Image.open(imgl)
    width, height = pic.size
    free = width/2 * freedom
    ...
    if(
        ...
        w = round(((pic_center[0]-img_center[0])/width) * 1000)/1000
        h = round(((pic_center[1]-img_center[1])/height) * 1000)/1000
        result = [w, h]
    print('')

    speak("I found the " + ob)
...
def getVoice():
    ...
    try:
        ...
        print(text)
        if "search" or "carol" in text :
            type(len(text))
            response = text[-1]
        # elif text == "carol scan the area":
        #     response = "okay scanning"
        # elif text == "carol scan emotions":
        #     response = "Ok I shall scan now"
        else:
